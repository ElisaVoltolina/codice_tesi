...
def solve_darp(V, PHOSP, DHOSP, HOSP, PHOME, DHOME, HOME, P, D, PD, idx, n, t, s, e, l, T, q, Q, a,b, p,  penalty_weights=None):
    
    model = Model("DARP")

    # Se non vengono forniti i pesi delle penalità, usa valori default
    #poi posso privilegarne alcune
    ...
    z=model.addVars([(i,j) for i in HOME for j in range(len(a[i]))], vtype=GRB.BINARY, name="z") #vsriabile binaria z[i][j] è uno se per il nodo i viene selezionata la time window j
    ...
    obj_timewindow_weights= quicksum(p[i][j]* z[i,j] for i in HOME for j in range(len(a[i]))) #peso time window selezionate per i nodi casa

    model.setObjective(obj_travel_time + obj_waiting_time + obj_penalty+ obj_timewindow_weights, GRB.MINIMIZE) 
    ...
    #fino a qui ok nulla da modificare


    #per ogni nodo casa seleziono sempre una time window (solo se serivita)
    for i in PHOME:
        # Per pickup casa
        model.addGenConstrIndicator(y[i], True,quicksum(z[i, j] for j in range(len(a[i]))) == 1, name=f"select_timewindow_pickup_home_{i}")
        model.addGenConstrIndicator(y[i], True,quicksum(z[i+n+n//2, j] for j in range(len(a[i+n+n//2]))) == 1, name=f"select_timewindow_delivery_home_{i}")
    ...
    for i in V:
        ...
        elif i in HOME: #per i nodi casa devo usare le time window multiple
            for j in range(len(a[i])):
                    model.addGenConstrIndicator(z[i, j], True, A[i] >= a[i][j], name=f"home_tw_early_{i}_{j}")  #così però non sto verificando che la richiesta sia servita
                    model.addGenConstrIndicator(z[i, j], True, A[i] <= b[i][j], name=f"home_tw_late_{i}_{j}")
    ...
    for i in V:
        if i==0 or i==2*n +1: #per i depositi i vincoli sono sempre attivi
            ...
            for j in range(len(a[i])):
                    model.addGenConstrIndicator(z[i, j], True, B[i] >= A[i],  name=f"home_B_early_home_{i}_{j}")
                    model.addGenConstrIndicator(z[i, j], True, B[i] >= a[i][j], name=f"home_B_tw_early_home{i}_{j}")
                    model.addGenConstrIndicator(z[i, j], True, B[i] <= b[i][j], name=f"home_B_late_home_{i}_{j}")
   ...
   #se imposto a zero quando non servita diventa infattibile PERCHè
    ...
    for i in V:
        if i in HOME:
            for j in range(len(a[i])):
                model.addGenConstrIndicator(z[i,j], True, B[i]>= a[i][j], name="time window1_home" )
                model.addGenConstrIndicator(z[i,j], True, B[i]>= b[i][j], name="time window1_home" )
        else:   #depositi e ospedali
            model.addConstr((B[i] >= e[i] ), name="timewindow1")
            model.addConstr((B[i] <= l[i] ), name="timewindow2")  
    ...
    if model.status == GRB.OPTIMAL or model.status == GRB.TIME_LIMIT:
        ...
        solution_z = {} #time window selezionata per i nodi casa
        for i in HOME: 
            for j in range(len(a[i])):
                if z[i, j].X > 0.5:
                    solution_z[i] = j
                    print(f"Nodo casa {i}: selezionata time window {j} [{a[i][j]}-{b[i][j]}] peso={p[i][j]}")
        ...
        for i in V:
            if i in HOME and i in solution_z:
                j = solution_z[i]
                print(f"Nodo {i}: {solution_A[i]:.2f} (finestra selezionata: {a[i][j]}-{b[i][j]}, peso={p[i][j]})")
            else:
        ...
        for i in V:
            if i in HOME and i in solution_z:
                j = solution_z[i]
                print(f"Nodo {i}: {solution_B[i]:.2f} (finestra selezionata: {a[i][j]}-{b[i][j]}, peso={p[i][j]})")
            else:
